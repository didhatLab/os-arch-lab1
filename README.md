# Eco.Lab1 - Интеграция компонентов калькулятора

## Обзор проекта

Проект демонстрирует различные подходы к интеграции компонентов в архитектуре Eco OS:
- **Агрегирование** (Aggregation) - включение компонентов через QueryInterface
- **Делегирование** (Delegation) - вызов методов через внутренние компоненты

## Архитектура компонентов


### 📊 Компоненты калькулятора

| Компонент | CID | Назначение | Методы | Подход |
|-----------|-----|------------|--------|--------|
| **CalculatorA** | `4828F6552E4540E78121EBD220DC360E` | Cложение/вычитание | - | Используется внутри D калькулятора |
| **CalculatorB** | `AE202E543CE54550899603BD70C62565` | Сложение/Вычитание | Addition, Subtraction | Агрегирование |
| **CalculatorC** | `4828F6552E4540E78121EBD220DC360E` | Сложение/Вычитание | Addition, Subtraction | Делегирование |
| **CalculatorD** | `3A8E44677E82475CB4A3719ED8397E61` | Умножение/Деление | Multiplication, Division | Агрегирование |
| **CalculatorE** | `872FEF1DE3314B87AD44D1E7C232C2F0` | Умножение/Деление | Multiplication, Division | Делегирование |

## Методы интерфейса IEcoLab1

### 🧮 Методы калькулятора (делегирование)

```c
/* Делегирование к CalculatorC */
int32_t Addition(int16_t a, int16_t b);      // → CalculatorC
int16_t Subtraction(int16_t a, int16_t b);   // → CalculatorC

/* Делегирование к CalculatorE */
int32_t Multiplication(int16_t a, int16_t b); // → CalculatorE
int16_t Division(int16_t a, int16_t b);       // → CalculatorE
```

### 🔄 Методы сортировки

```c
int16_t BucketSortInt(int* array, uint32_t length);
int16_t BucketSortLong(long* array, uint32_t length);
int16_t BucketSortFloat(float* array, uint32_t length);
int16_t BucketSortDouble(double* array, uint32_t length);
int16_t BucketSortLongDouble(long double* array, uint32_t length);
```

## Результаты тестирования

### ✅ Успешные тесты

```
EcoLab1 unit tests
Direct Y: 2*7=14                                    ← Прямой доступ к CalculatorD
CalculatorX: 3+5=8, 10-3=7                         ← Агрегирование CalculatorB
CalculatorY: 4*6=24, 15/3=5                        ← Агрегирование CalculatorD
Delegated CalculatorC+E: 7+8=15, 20-7=13, 6*9=54, 24/4=6  ← Делегирование
```

### 🎯 Архитектурные паттерны

1. **Агрегирование через QueryInterface**:
   ```c
   // Получение CalculatorB через QueryInterface
   pIEcoLab1->pVTbl->QueryInterface(pIEcoLab1, &IID_IEcoCalculatorX, &pIX);
   pIX->pVTbl->Addition(pIX, 3, 5);  // Результат: 8
   ```

2. **Делегирование методов**:
   ```c
   // Прямой вызов методов через CEcoLab1
   pIEcoLab1->pVTbl->Addition(pIEcoLab1, 7, 8);      // → CalculatorC: 15
   pIEcoLab1->pVTbl->Multiplication(pIEcoLab1, 6, 9); // → CalculatorE: 54
   ```

3. **Прямой доступ к компонентам**:
   ```c
   // Создание CalculatorD через интерфейсную шину
   pIBus->pVTbl->QueryComponent(pIBus, &CID_EcoCalculatorD, 0, &IID_IEcoCalculatorY, &pIY);
   pIY->pVTbl->Multiplication(pIY, 2, 7);  // Результат: 14
   ```

## Переменные окружения

Перед запуском скрипта `start.sh` необходимо установить переменную окружения `ECO_FRAMEWORK`:

```bash
export ECO_FRAMEWORK=/path/to/Eco.Framework
```

Например:
```bash
export ECO_FRAMEWORK=/home/dan/shit/Eco.Framework
```

## Запуск

```bash
cd /path/to/Eco.Lab1
./start.sh
```

## Что делает скрипт

1. **Проверяет** наличие переменной `ECO_FRAMEWORK`
2. **Очищает** старые файлы сборки
3. **Собирает** все компоненты:
   - CalculatorA (статические библиотеки)
   - CalculatorB (статические библиотеки)
   - CalculatorD (статические библиотеки)
   - CalculatorC (статические библиотеки)
   - CalculatorE (статические библиотеки с зависимостью от CalculatorB)
   - CEcoLab1 (основной компонент)
   - Юнит-тест (исполняемый файл)
4. **Запускает** тесты
5. **Очищает** временные файлы

## Требования

- GCC компилятор
- Eco.Framework установлен и доступен
- Переменная `ECO_FRAMEWORK` указывает на корень фреймворка

## Технические детали

### 🔧 Статические библиотеки

- `lib1F5DF16EE1BF43B999A434ED38FE8F3A.a` - CEcoLab1
- `libAE202E543CE54550899603BD70C62565.a` - CalculatorB
- `lib4828F6552E4540E78121EBD220DC360E.a` - CalculatorA/C
- `lib3A8E44677E82475CB4A3719ED8397E61.a` - CalculatorD
- `lib872FEF1DE3314B87AD44D1E7C232C2F0.a` - CalculatorE

### 📁 Структура файлов

```
Eco.Lab1/
├── SourceFiles/           # Исходный код CEcoLab1
├── HeaderFiles/           # Заголовочные файлы
├── UnitTestFiles/         # Юнит-тесты
├── AssemblyFiles/         # Makefile'ы для сборки
├── Eco.CalculatorA/       # Компонент A
├── Eco.CalculatorB/       # Компонент B (агрегирование)
├── Eco.CalculatorC/       # Компонент C (делегирование)
├── Eco.CalculatorD/       # Компонент D (агрегирование)
├── Eco.CalculatorE/       # Компонент E (делегирование)
└── start.sh              # Скрипт сборки и тестирования
```

## Заключение

Проект успешно демонстрирует:
- ✅ Интеграцию множественных компонентов калькулятора
- ✅ Различные подходы к композиции (агрегирование vs делегирование)
- ✅ Работу с интерфейсной шиной Eco OS
- ✅ Статическую компоновку компонентов
- ✅ Полную автоматизацию сборки и тестирования

Все методы калькулятора работают корректно через различные архитектурные паттерны, что подтверждает гибкость и надежность компонентной архитектуры Eco OS.