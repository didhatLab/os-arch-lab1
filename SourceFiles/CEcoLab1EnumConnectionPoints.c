...
int16_t ECOCALLMETHOD CEcoLab1EnumConnectionPoints_Next(/* in */ struct IEcoEnumConnectionPoints* me, /* in */ uint32_t cConnections, /* out */ struct IEcoConnectionPoint **ppCP, /* out */ uint32_t *pcFetched) {
    CEcoLab1EnumConnectionPoints* pCMe = (CEcoLab1EnumConnectionPoints*)me;
    struct IEcoConnectionPoint* pCP = 0;
    uint32_t count = 0;

    if (me == 0 || ppCP == 0 || (pcFetched == 0 && cConnections > 1)) {
        return -1;
    }

    while ((pCMe->m_cIndex < pCMe->m_List->pVTbl->Count(pCMe->m_List)) && (count < cConnections)) {
        pCP = (struct IEcoConnectionPoint*)pCMe->m_List->pVTbl->Item(pCMe->m_List, pCMe->m_cIndex);
        if (pCP != 0) {
            pCP->pVTbl->AddRef(pCP);
            ppCP[count] = pCP;
            count++;
        }
        pCMe->m_cIndex++;
...
int16_t ECOCALLMETHOD CEcoLab1EnumConnectionPoints_Clone(/* in */ struct IEcoEnumConnectionPoints* me, /* out */ struct IEcoEnumConnectionPoints **ppEnum) {
    ...
    /* Получаем первый connection point из списка для клонирования */
    if (pCMe->m_List->pVTbl->Count(pCMe->m_List) > 0) {
        pCP = (struct IEcoConnectionPoint*)pCMe->m_List->pVTbl->Item(pCMe->m_List, 0);
        return createCEcoLab1EnumConnectionPoints((IEcoUnknown*)pCMe->m_pISys, pCP, ppEnum);
    }
